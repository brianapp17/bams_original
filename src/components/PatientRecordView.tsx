...
const PatientRecordView: React.FC = () => {
  ...
  const [showResourceMenu, setShowResourceMenu] = useState(false); // State to control the visibility of the resource menu
  const [resourceToAdd, setResourceToAdd] = useState<string | null>(null); // State to track which resource form to show

  const auth = getAuth(app);
  const database = getDatabase(app);

  // Effect to fetch patient data and medical records
  useEffect(() => {
  ...
  const handleSelectResource = (resourceType: string) => {
    ...
    setResourceToAdd(resourceType);
    setShowResourceMenu(false); // Close the menu after selection
    console.log(`handleSelectResource: showResourceMenu set to false, resourceToAdd set to ${resourceType}`);
  };

  const handleCloseForm = () => {
    console.log('handleCloseForm: Setting resourceToAdd to null and showResourceMenu to false');
    setResourceToAdd(null);
    // Also ensure the menu is hidden when a form is closed without saving
    setShowResourceMenu(false); 
  };

  // Placeholder Save handlers for each form type
  const handleSaveObservation = (formData: any) => {
    console.log('Saving Observation data:', formData);
    // TODO: Implement API call to save Observation
    handleCloseForm();
  };

  const handleSaveCondition = (formData: any) => {
    console.log('Saving Condition data:', formData);
    // TODO: Implement API call to save Condition
    handleCloseForm();
  };

  const handleSaveEncounter = (formData: any) => {
    console.log('Saving Encounter data:', formData);
    // TODO: Implement API call to save Encounter
    handleCloseForm();
  };

  const handleSaveProcedure = (formData: any) => {
    console.log('Saving Procedure data:', formData);
    // TODO: Implement API call to save Procedure
    handleCloseForm();
  };

  const handleSaveMedicationAdministration = (formData: any) => {
    console.log('Saving Medication Administration data:', formData);
    // TODO: Implement API call to save Medication Administration
    handleCloseForm();
  };

   // Placeholder Save handler for the duplicated form
   const handleSaveMedicationAdministrationDuplicate = (formData: any) => {
    console.log('Saving Duplicated Medication Administration data:', formData);
    // TODO: Implement API call to save Duplicated Medication Administration
    handleCloseForm(); // Close form after saving
  };

    console.log(`Rendering PatientRecordView: showResourceMenu=${showResourceMenu}, resourceToAdd=${resourceToAdd}`);
      ...
      <div className="min-h-screen bg-gray-50 flex">
        ...
        {showResourceMenu && (
          <AddResourceMenu
            ...
            onCancel={() => setShowResourceMenu(false)}
          />
        )}

        {/* Render the selected resource form */}
        {resourceToAdd === 'Observation' && <AddObservationForm onSave={handleSaveObservation} onCancel={handleCloseForm} />}
        {resourceToAdd === 'Condition' && <AddConditionForm onSave={handleSaveCondition} onCancel={handleCloseForm} />}
        {resourceToAdd === 'Encounter' && <AddEncounterForm onSave={handleSaveEncounter} onCancel={handleCloseForm} />}
        {resourceToAdd === 'Procedure' && <AddProcedureForm onSave={handleSaveProcedure} onCancel={handleCloseForm} />}
        {resourceToAdd === 'MedicationAdministration' && <AddMedicationAdministrationForm onSave={handleSaveMedicationAdministration} onCancel={handleCloseForm} />}

        {/* Conditional rendering for the duplicated form */} 
        {resourceToAdd === 'MedicationAdministrationDuplicate' && (
