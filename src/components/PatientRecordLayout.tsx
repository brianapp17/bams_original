...
const PatientRecordLayout: React.FC = () => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        if (patientId) {
            ...
            const patientListener = onValue(patientRef, (snapshot) => {
              ...
              if (patientData) {
                setPatient(patientData);
                 // Map patient data to PatientInfo type for the sidebar
                setPatientInfo({
            ...
            // No patientId in the URL, show a message to select a patient
            setPatient(null);
            setPatientInfo(null);
            setMedicalRecords(null);
            setPatientError(null);
  ...
  const handleSearch = (e: React.FormEvent) => { 
      e.preventDefault();
      // TODO: Implement actual filtering of medicalRecords based on searchQuery
      console.log('Search submitted:', searchQuery);
      // For now, search is handled within MedicalRecords component if you pass searchQuery down
      // You might need to lift filtering logic or pass search query down to MedicalRecords
  };
  const resetSearch = () => { 
      setSearchQuery(''); 
      setSelectedCategory(null); 
      // TODO: Reset MedicalRecords filtering
  };
   // Note: selectedCategory and setSelectedCategory are passed to MedicalRecords


  // Handle patient selection (used by PatientSelector - which is removed, but kept here for completeness if re-added or similar logic needed)
  const handlePatientSelect = (id: string) => {
      navigate(`/expedientes/${id}`); // Navigate to the specific patient record page
  }


  // Display loading state for the overall layout
  if (isLoadingPatient && patientId) {
   ...
   // Display message if no patient is selected at the root path
  ...
  // Display error if patient not found or other loading error
  if (patientError || !patient) {
   ...
   // Render the layout with sidebars and central content
  return (
    <div className="min-h-screen bg-gray-50 flex">
        ...
        <PatientSidebar
              ...
              selectedCategory={selectedCategory ? getCategoryLabel(selectedCategory) : null} // Assuming getCategoryLabel is available or logic moved here
              setSelectedCategory={setSelectedCategory}
              fhirData={fhirDataString} // Pass fetched medical records data as fhirData
            />

      {/* Main Content Area (Patient Record Details and Medical Records) */}
      <div className="flex-1 mx-auto max-w-8xl px-4 py-8">
         {/* Back button is now inside the main content area */}
           <div className="mb-8">
                <Link to="/expedientes" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-teal-600 hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500">
         ...
         {/* Patient Basic Information */}
            <div className="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md mb-6">
                ...
                <div className="border-b pb-4 mb-4">
                 ...
                 {/* Search Bar for Medical Records - Keep this within the central content */}
                <SearchBar 
                ...
                <h2 className="text-xl font-bold text-teal-800 mb-4 mt-8">Registros Médicos</h2>
                 {isLoadingRecords ? (
                     <div className="text-center text-gray-600">Cargando registros médicos...</div>
                      ...
                      {/* Pass medicalRecords and search/category states to MedicalRecords */}
      ...
      <div className="w-96 flex-shrink-0"></div> {/* Placeholder space */}
       <ChatSidebar
              ...
              resultsData={fhirDataString} // Pass fetched medical records data
              selectedPatientId={patientId || null} // Pass the current patientId
            />
    </div>
  );
};
